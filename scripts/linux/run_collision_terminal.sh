...
COLLISION_CONDA_EXE="${CRK_CONDA_EXE:-${CONDA_EXE:-conda}}"
CONDA_BASE="$("${COLLISION_CONDA_EXE}" info --base)"
source "${CONDA_BASE}/etc/profile.d/conda.sh"
conda activate "${CRK_CONDA_ENV:-AUV}"
export MPLBACKEND=Agg OMP_NUM_THREADS=1 MKL_NUM_THREADS=1
cd "${REPO_ROOT}"
exec python -B -m chapter3_bser.experiments.phase1c_prrac.collision_terminal_cli "$@"
