...
class ClosedLoopDivergenceTests(unittest.TestCase):
    def test_same_residual_with_local_overlay_changes_only_searcher_trajectory(self):
        config = evaluation_config(scenario_count=1); config["max_steps"] = 5
        ...
        c0_env = evaluator._make_env(config, reward); c2_env = evaluator._make_env(config, reward)
        try:
            c0_env.reset(scenario=scenario, episode_id=0, episode_index=0)
            c2_env.reset(scenario=scenario, episode_id=0, episode_index=0)
            phase_config = load_phase1b2_config(); runtime = execution_runtime_config(config)
            phase_config["execution_runtime"] = runtime
            provider = OnlinePlanningStateProvider(c2_env, refresh_interval=20,
            ...
            state = provider.initialize(); context = evaluator._public_context(c2_env, state)
            base_controller = evaluator._build_episode_controller(phase_config, config,
                execution_variant=ExecutionVariant.B0_LEGACY_V2_1,
                runtime_integration_mode="legacy", checkpoint_runtime_revision="dynamic_public_intercept_v2_1")
            initialized = base_controller.initialize(state, context)
            base = RMADDPGGuidanceBridge().compile_guidance(initialized.allocation, state, context, decision_reason="INITIALIZE")
            recovery = build_search_recovery_controller(SearchRecoveryVariantV2.S2A1_C2_LOCAL_CONNECTOR)
            recovery.observe_transition(stage_before=0, planning_state_before=state, planning_state_after=state,
                                        installed_guidance_before=base, collision_flags=(False, False, False, False))
            collision_state = replace(state, step=1)
            recovery.observe_transition(stage_before=0, planning_state_before=state, planning_state_after=collision_state,
                                        installed_guidance_before=base, collision_flags=(True, False, False, False))
            recovery.prepare_next_guidance(collision_state, base)
            next_collision_state = replace(state, step=2)
            recovery.observe_transition(stage_before=0, planning_state_before=collision_state, planning_state_after=next_collision_state,
                                        installed_guidance_before=base, collision_flags=(True, False, False, False))
            recovery.prepare_next_guidance(next_collision_state, base)
            overlay = apply_search_recovery_guidance(base, next_collision_state, recovery)
            ...
            c0_env.install_guidance(base); c2_env.install_guidance(overlay)
            c0_env.refresh_observation_after_guidance(); c2_env.refresh_observation_after_guidance()
            residual = torch.zeros((4, 3), dtype=torch.float32)
            for _ in range(3):
                c0_env.step(residual.clone()); c2_env.step(residual.clone())
