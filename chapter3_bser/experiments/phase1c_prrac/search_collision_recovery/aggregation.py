...
def _paired_groups(rows: list[dict[str, Any]]):
    keys = ("checkpoint", "checkpoint_config_hash", "checkpoint_runtime_revision", "evaluation_runtime_revision", "runtime_integration_mode", "execution_variant", "evaluation_mode", "manifest_sha256", "search_collision_recovery_config_hash")
...
def paired_search_collision_recovery_comparisons(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    output = []
    order = [item.value for item in SearchRecoveryVariant]
    for root, variants in sorted(groups.items(), key=lambda item: str(item[0])):
        ...
        for left_name, right_name in itertools.combinations(available, 2):
            ...
            if {int(row["scenario_seed"]) for row in left.values()} != {int(row["scenario_seed"]) for row in right.values()}:
                raise ValueError("paired search recovery variants require identical scenario_seed sets")
            ...
            for label, field in (("pre_found_collision_count", "searcher_collision_count_pre_found_total"), ("pre_found_collision_max_streak", "searcher_collision_max_streak_pre_found"), ("found_step", "found_step"), ("searcher_distance_travelled", "searcher_distance_travelled_pre_found"), ("map_known_fraction_gain", "map_known_fraction_gain_pre_found")):
...
def paired_search_collision_recovery_baseline_strata(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    baseline_name = SearchRecoveryVariant.S2A_C0_BASELINE.value
    for root, variants in sorted(groups.items(), key=lambda item: str(item[0])):
        if baseline_name not in variants: continue
        baseline = {str(row["scenario_id"]): row for row in variants[baseline_name]}
        for candidate_name in (SearchRecoveryVariant.S2A_C1_ROUTE_REFRESH.value, SearchRecoveryVariant.S2A_C2_EGRESS_ROUTE.value):
            ...
            for stratum, collision in (("BASELINE_COLLISION", True), ("BASELINE_NO_COLLISION", False)):
                ...
                item=dict(zip(keys,root)); item.update({"baseline_search_recovery_variant":baseline_name,"candidate_search_recovery_variant":candidate_name,"stratum":stratum,"stratum_definition":"S2A_C0_BASELINE.searcher_collision_episode_pre_found","scenario_count":len(ids)})
